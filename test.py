...
img = cv2.imread("./Icons/areaLight.png", cv2.IMREAD_UNCHANGED)
...
imageProcessing[blockspergrid, threadsperblock](d_img, original_shape[0], original_shape[1], original_shape[2], blurIntensity)
img = d_img.copy_to_host()

img = numpy.reshape(img, original_shape)
cv2.imshow("image", img)
cv2.waitKey()
